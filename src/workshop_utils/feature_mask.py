...
def create_feature_mask(
    ...
    block_sizes = [
        b.n_output_features for b in design_matrix_basis._iterate_over_components()
    ]
    if isinstance(block_structure, str):
        block_structure = len(block_sizes) * [block_structure]
    if len(block_sizes) != len(block_structure):
        raise ValueError(
            "block_structure length not the same as the number of blocks defined by design_matrix_basis!"
            f" len(block_structure): {len(block_structure)}, len(design_matrix_basis): {len(block_sizes)}"
        )
    if any(
        [b not in ["all", "none", "self"] and not callable(b) for b in block_structure]
    ):
        bad_vals = set(
            [
                b
                for b in block_structure
                if b not in ["all", "none", "self"] and not callable(b)
            ]
        )
        raise ValueError(
            f"block_structure values must be one of 'all', 'none', 'self' or a function, but got {bad_vals}!"
        )
    shape_sum = 0
    for s, b in zip(block_sizes, block_structure):
        if b == "all":
            ...
            mask[shape_sum : shape_sum + s] = np.repeat(
                np.eye(n_neurons), s // n_neurons, 0
            )
        elif callable(b):
            m = b(s, n_neurons)
            if (m.astype(bool).astype(float) != m).any():
                raise ValueError(
                    f"block_structure callable {b} must return a binary array!"
                )
...
def create_feature_mask_paramgrid(
    basis: AdditiveBasis,
    ...
    for b1 in basis1_n_basis_funcs:
        ...
        param_grid.append(
            {
                "glm__feature_mask": [
                    create_feature_mask(basis, ["all", "none"], n_neurons=n_neurons)
                ],
                "basis__basis1__n_basis_funcs": [b1],
                "basis__basis2__n_basis_funcs": [basis2_n_basis_funcs[0]],
            }
        )
    ...
    for b2 in basis2_n_basis_funcs:
        ...
        param_grid.append(
            {
                "glm__feature_mask": [
                    create_feature_mask(basis, ["none", "all"], n_neurons=n_neurons)
                ],
                "basis__basis1__n_basis_funcs": [basis1_n_basis_funcs[0]],
                "basis__basis2__n_basis_funcs": [b2],
            }
        )
    ...
    for b1 in basis1_n_basis_funcs:
        for b2 in basis2_n_basis_funcs:
            ...
            param_grid.append(
                {
                    "glm__feature_mask": [
                        create_feature_mask(basis, "all", n_neurons=n_neurons)
                    ],
                    "basis__basis1__n_basis_funcs": [b1],
                    "basis__basis2__n_basis_funcs": [b2],
                }
